...
export default function nonCarOwnerRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user) {
      try {
        await setDoc(doc(db, 'nonCarOwners', user.uid), {
          email: user.email,
          floorNo,
          department,
          departureTime,
        ...
        window.location.href = '/nonCarOwner/dashboard'; // Change this to the appropriate page after registration
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-catalinaBlue">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full space-y-4">
        <h2 className="text-2xl font-bold mb-4">Non Car-owner Details Registration</h2>
